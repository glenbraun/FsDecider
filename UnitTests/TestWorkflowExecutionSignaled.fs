...
module TestWorkflowExecutionSignaled =
    let private OfflineHistorySubstitutions =  
        ...
        |> Map.add "Input" "signalInput"

    let ``Workflow Execution Signaled with result of Signaled``() =
        let workflowId = "Workflow Execution Signaled with result of Signaled"
        ...
        let deciderFunc(dt:DecisionTask) =
            ...
            let! signal = FsDeciderAction.WorkflowExecutionSignaled(signalName)
        ...
        let offlineFunc = OfflineDecisionTask (TestConfiguration.WorkflowType) (WorkflowExecution(RunId="Offline RunId", WorkflowId = workflowId))
                          ...
                          |> OfflineHistoryEvent (        // EventId = 5
                              WorkflowExecutionSignaledEventAttributes(Input=signalInput, SignalName=signalName))
        ...
        for (i, resp) in TestHelper.PollAndDecide (TestConfiguration.TaskList) deciderFunc offlineFunc false 2 do
            ...
            | 1 -> 
                ...
                TestHelper.SignalWorkflow runId workflowId signalName signalInput
    ...
    let ``Workflow Execution Signaled with result of NotSignaled``() =
        let workflowId = "Workflow Execution Signaled with result of NotSignaled"
        let signalName = "Test Signal"
        let signalInput = "Test Signal Input"
